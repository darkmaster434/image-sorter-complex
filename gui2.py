...
c_path='C:/Users/Corrupted/Desktop/down' # my given directory
...
class MuGUI():
    ...
    def __init__(self) -> None:
        if os.path.exists(cc_path):
            c_o=0
            c_n_o=0
            for i in os.listdir(cc_path):
                if i.find('optimize'):
                    c_o+=len(os.listdir(os.path.join(cc_path,i)))
                else:
                    c_n_o+=len(os.listdir(os.path.join(cc_path,i)))
                print(i,":",len(os.listdir(os.path.join(cc_path,i))))
            print(f'**********************************\nTotal Sorted:{c_o+c_n_o}\nTotal Optimize:{c_o}\nTotal Not Optimize:{c_n_o}\n**********************************\n ')
        global img1,img2
        self.root=tk.Tk()
        ...
        self.make_dir=tk.Button(b_frame,text='Make Directory',command=self.mkdir,background='cyan')
